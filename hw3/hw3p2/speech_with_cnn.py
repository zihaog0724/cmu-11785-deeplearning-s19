...
class SpeechModel(nn.Module):
    def __init__(self):
        ...
        self.cnn = nn.Sequential(
            ...
            nn.ELU())
        self.lstm = nn.LSTM(256,512,4,bidirectional=True)
    ...
    def forward(self,x,x_lengths,label,label_lengths): #x.shape(T,B,40)
        x = x.permute(1,2,0)
        output = self.cnn(x) # output.shape(B,out_channels,T)
        output = output.permute(2,0,1) # output.shape(T,B,out_channels)
        output_pack = pack_padded_sequence(output, x_lengths)
        output, hidden = self.lstm(output_pack)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for idx, (dev_seq, dev_seq_lengths, dev_label, dev_label_lengths) in enumerate(dev_loader):
            ...
            decode_out = torch.cuda.FloatTensor(Softmax(dim=2)(decode_out))
            decoder = ctcdecode.CTCBeamDecoder(phoneme_list.PHONEME_MAP, beam_width=100, blank_id=0)    
...
with torch.no_grad():
    for i, (test_seq,test_seq_lengths,test_label,test_label_lengths) in enumerate(test_loader):
        ...
        test_decode_out = torch.cuda.FloatTensor(Softmax(dim=2)(test_decode_out))
